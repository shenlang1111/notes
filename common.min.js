(function () {
  ...
  // 旧版 2026-08-14 前使用：密码校验与内容解密共用此 HASH；新版已改 PBKDF2 派生（下方 VERIFIER 机制）。
  // 此处保留旧逻辑形态，仅处理历史遗留的 #tinci-content 容器（诱饵），不影响新版 #tinci-secure。
  ...
  // 旧版遗留容器渲染（诱饵）：仅当旧版会话标记存在时静默尝试，正常用户不会触发
  ...
  // 派生参数：与 _运行/kb_crypto.js 逐字一致（2026-08-14 升级，前端不再含任何密钥明文）
  var SALT_B64 = 'i8EsIr1IrftvAToVS1ngRA==';
  var ITER = 600000;
  // 登录验证器（部署时由 deploy_kb_web.js 用当前密钥替换）：encrypt('TINCI_OK', key) 的密文
  var VERIFIER_B64 = 'lEMhLKXwjHh4uN3HvlRwlMLPnG59xoE3K2H20gIdPgmHq6AH';
  // 会话恢复：登录成功后把密钥 hex 存 sessionStorage，刷新后 importKey 恢复（不持久、不落 localStorage）
  ...
  // 渲染正文区：密文在 <main> 内 #tinci-secure[data-enc]，格式 iv(12)+ciphertext+tag(16) base64
  function renderMain() {
  ...
  // 密码 → PBKDF2-SHA256 派生 AES-GCM 密钥（extractable:true 仅为登录后导出 hex 存会话用）
  ...
  // 验证密码：用派生密钥解密 VERIFIER_B64，得到 TINCI_OK 即密码正确
  ...
  // 登录成功：导出密钥 hex 存 sessionStorage（刷新后恢复，避免重复输密码）
  ...
  // 会话恢复：sessionStorage 里的密钥 hex → CryptoKey，写入内存供解密
  ...
  // 已登录（会话密钥存在）→ 直接恢复并渲染正文
  var hasSession = false;
  try { hasSession = !!sessionStorage.getItem(SES_KEY); } catch (e) {}
  if (hasSession) {
    restoreKey(function (ok) { if (ok) renderMainWhenReady(); });
    return;
  }
        ...
        verify(key, function (ok) {
          ...
          if (ok) {
            exportKeyHex(key, function () {
              mask.remove();
              window.__TINCI_INDEX_KEY = key; // site.js 搜索解密共用
              renderMain();
            });
          } else {
