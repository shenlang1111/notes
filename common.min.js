(function () {
  ...
  var FLAG = HASH.slice(0, 8); // 会话凭证片段（解锁通过的标记）
  ...
  // 内容解密密钥（解锁后写入内存，不落 localStorage）
  ...
  // 解锁通过后：把密钥 importKey 成 CryptoKey 写入内存（WebCrypto 要求 key 必须是 CryptoKey，
  ...
  function build() {
    ...
    function check() {
      ...
      crypto.subtle.digest('SHA-256', new TextEncoder().encode(v)).then(function (buf) {
        ...
        if (hex === HASH) {
          sessionStorage.setItem('tinci_auth_ok', FLAG); // 凭证存会话
          mask.remove();
          unlockContent(renderMain); // 解锁后渲染正文
        } else {
