...
| 日期 | 新增机制 | 去哪读 | 谁加的 |
|---|---|---|---|
| 2026-08-03 | **对话存档自动化（闲聊 AI 落地，用户拍板"你来落地吧"）**——新建 `tools/chat_autosave.js`：SessionEnd 钩子自动扫 `D:\ai\brain-memory\chat_inbox\chat-<agent>-<日期>.md` → 调 mem_chat_save 入库（自动切段）→ 归档 archive/。**所有 AI 知悉**：①**存完整对话记录，不是摘要**（用户 2026-08-03 拍板：每次聊完/每 5 轮，把自上次存档以来的逐轮对话写进 `chat_inbox/chat-<agent>-<日期>.md`，钩子自动入库；AI 复述为完整覆盖每轮内容，接近原文非逐字）②原 `mem_chat_save --text` 方式仍可用（钩子是补充不是替代）③提炼交给大脑 evolve_auto（完整记录入库后自动提炼成经验，存=全量原文、炼=自动提炼） | tools/chat_autosave.js（新）+ .claude/settings.json + chat_inbox（新目录） | 闲聊 AI |
