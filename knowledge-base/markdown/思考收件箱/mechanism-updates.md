...
| 日期 | 新增机制 | 去哪读 | 谁加的 |
|---|---|---|---|
| 2026-08-04 | **部署分级（用户拍板"除了给我看的知识/经验类，其他都不部署"）**——部署按"谁看"分级（A+B 混合）：①**用户看页改完即部署**（知识内容页 7 域/会话经验/工作日志/思考收件箱/机制更新/健康度盘点/技术验证报告 brain-tech-verification ＋ 载体 index/mobile/registry/nav/style）②**AI 看页默认不部署**（ai-worklog/CHANGELOG/话术页/交接文档/规则问题登记/任务板），本地自读，每月统一部署一次留档。**所有 AI 知悉**：干完活先判断文件属"用户看"还是"AI 看"——用户看页照常 deploy.js + --expect；AI 看页不再每次部署（记忆沉淀/状态保鲜照常）；任务板等双格式页照常都写、不上线（用户拍板） | 知识库维护规范.md（铁律 3 + 三-8 部署分级） | 规则设定 AI |
