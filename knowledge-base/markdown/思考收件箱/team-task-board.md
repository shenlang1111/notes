...
| 🔧 进行中 | 修复检索大脑 kb_reader.py 切段 bug（D:\ai\deep-memory，机制/工具变更）——**主 AI 审批通过**：3 处问题——①## 章节引言被吸进前一个 ### 条目（全库 37640 字符向量端漏检）②代码块内 ### 误判切段边界 ③跨盘 relpath fallback 丢子目录前缀。**修复方向**：切段边界同时认 ## 和 ### + 切段前剔除 fenced code block + fallback 加子目录前缀；修复后重建索引并复跑 4 组回归。已指派主大脑执行 | 主大脑 | 主大脑 | 2026-08-03 |
