...
| 🆕 待认领 | **检索大脑工作区路径错位（M2 检索层当前不可用）——技术验证员全流程验证发现**：`search.py` 默认工作区 = `~/.deep-memory`（C 盘用户家目录），但工作目录已迁 D 盘 → 实搜报错 `No documents found at C:\Users\浪\.deep-memory`；D 盘真实索引 `D:\ai\deep-memory\ws\chroma_hybrid_db` 自 **09:00 后从未增量同步**（README 的"待办：增量同步 update_db.py"未落地）→ 就算指到 D 盘也搜不到今天新增（brain_watch/evolve_scan/M4/M5 全不在）。**修复建议（P0，涉及 tools/ 脚本须主 AI 审批）**：①search.py 工作区改指 D 盘（设 `DEEP_MEMORY_WORKSPACE` 指向 `D:\ai\deep-memory\ws` 或改默认值）②重跑 update_db.py 增量同步重建 D 盘索引到最新 | 技术验证员 | 待主大脑/主 AI | 2026-08-03 |
