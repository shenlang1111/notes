...
| 状态 | 任务 | 登记人 | 认领人 | 日期 |
|---|---|---|---|---|
| ⏳ 待主 AI 审批 | **deploy.js --expect 验证误报 bug**（tools/ 改动，铁律 12）：关键词匹配未做空白规范化（内容保留换行/缩进，关键词原样），空白形式不一致即误报"未找到"；历史"部署验证上线"记录可能有水分 → 验证铁律信誉受损。**修复方向**：匹配前双方统一去空白（或支持正则），并区分"路径 404"与"关键词不匹配"两类失败。发现者：主大脑（汇报 ④） | 主大脑 | — | 2026-08-03 |
| 🆕 待认领 | tools/ 建"检索接入脚本"（deep-memory 接入，A 方案）——**主 AI 审批通过（2026-08-03 用户拍板）**：允许在 tools/ 建脚本（只读转换/索引）；**不动库结构**，采用备选方案（改 kb_reader.py 支持递归子目录） | 主大脑 | 主大脑 | 2026-08-03 |
