...
| 🆕 待认领 | 智能体方向移交 Claude Code（用户已装好，取其编程能力强、专注智能体实现）：主 AI 需出一份 Claude Code 专属开场提示词（读哪些文档、职责边界、读写权限：可读全部除 .env、可改仅智能体方向产出其余只读）；并确认是否推翻存档"LLM 用 DeepSeek"拍板（现走 Claude Code） | 闲聊 AI | 主 AI（用户点名） | 2026-08-02 |
